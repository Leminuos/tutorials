...
### 2.1. Built-in driver hay In box driver

Đây là các driver được build chung cùng với OS trong quá trình build kernel để tạo ra image và nạp vào thẻ SD.

$\rightarrow$ đơn giản là thêm driver vào kernel trong lúc build.

Đặc điểm của driver này là chúng luôn có mặt khi hệ thống khởi động, không thể gỡ bỏ trong lúc hệ thống đang chạy.

Các driver này được sử dụng cho các thiết bị cần thiết để khởi động hệ thống, như driver cho ổ cứng, hệ thống file, bộ nhớ.

Khi build kernel, trong file cấu hình `.config`, các driver được chọn là built-in khi có dấu `=y`.
...
### 2.2. Loadable driver hay loadable kernel module
...
- Nếu kernel module gặp lỗi $\rightarrow$ crash kernel.
...
Khi làm mạch, ta mua SoC về và tải BSP từ hãng thì trong BSP đã có sẵn platform driver.
...
Khi lập trình userspace, ta có một môi trường rất thoải mái:
...
Khi viết driver thì ta đang viết code bên trong kernel $\rightarrow$ Không còn ai bảo vệ nữa.

### 3.1. Những thứ không tồn tại trong kernel space

**Không có libc.**

Các API như `printf()`, `malloc()`, `free()` từ libc không dùng được. Kernel có API riêng:
...
Nếu dùng sai flag, ví dụ dùng `GFP_KERNEL` trong interrupt handler thì kernel sẽ phát hiện và panic ngay lập tức.

### 3.2. Stack cố định và rất nhỏ
...
Trong kernel space, mỗi process/thread có một kernel stack cố định, không thể tự động mở rộng (thường chỉ 4KB hoặc 8KB tùy kiến trúc).
...
### 3.3. Không có exception handler
...
Trong kernel space, khi code của ta dereference NULL pointer:
...
Kernel sẽ in oops message với call stack ra `dmesg`, sau đó tùy cấu hình có thể tiếp tục chạy hoặc panic hoặc reboot. Không có cơ chế nào bắt lỗi và recover một cách sạch sẽ như userspace.

:::tip Mindset cần có
Trong kernel space, mọi pointer đều phải được kiểm tra trước khi dereference. Không có ngoại lệ.
:::

## 4. Lập trình phần cứng
...
Trên Linux, điều này không thể làm được từ userspace vì MMU đã bật, lúc này mọi địa chỉ trong code đều là địa chỉ ảo. Truy cập thẳng địa chỉ vật lý sẽ bị kernel chặn ngay.
...
### 4.1. ioremap: cầu nối giữa địa chỉ vật lý và ảo
...
### 4.2. Tại sao không dùng `*ptr` trực tiếp?
